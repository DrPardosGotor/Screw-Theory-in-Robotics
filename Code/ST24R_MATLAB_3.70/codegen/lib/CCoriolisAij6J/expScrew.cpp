...
void expScrew(const double TwMag[7], double H[16])
{
  ...
  if (y == 0.0) {
    //  only translation
    std::memset(&ws[0], 0, 9U * sizeof(double));
    ws[0] = 1.0;
    p[0] = TwMag[0] * TwMag[6];
    ws[4] = 1.0;
    p[1] = TwMag[1] * TwMag[6];
    ...
    //
    scale = std::sin(TwMag[6]);
    absxk = 1.0 - std::cos(TwMag[6]);
    for (i = 0; i < 9; i++) {
      b_I[i] = 0;
    }
    ...
    //
    for (i = 0; i < 9; i++) {
      ws[i] = (static_cast<double>(b_I[i]) + ws[i] * scale) + b_ws[i] * absxk;
      b_I[i] = 0;
    }
    ...
    b_I[8] = 1;
    for (i = 0; i < 9; i++) {
      b_ws[i] = static_cast<double>(b_I[i]) - ws[i];
    }
