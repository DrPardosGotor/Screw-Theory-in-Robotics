...
//                const double Thetap[7]
//                double Ctdt[49]
// Return Type  : void
//
void CCoriolisAij7J(const double TwMag[49], const double LiMas[49], const double
                    Thetap[7], double Ctdt[49])
{
  double Cosij;
  double d;
  ...
  //  Ctdt = CCoriolisAij(TwMag,LiMas,Thetap)
  //
  std::memset(&Ctdt[0], 0, 49U * sizeof(double));
  for (int i = 0; i < 7; i++) {
    for (int j = 0; j < 7; j++) {
      int b_i;
      b_i = i + 7 * j;
...
//
// File trailer for CCoriolisAij7J.cpp
//
// [EOF]
//
