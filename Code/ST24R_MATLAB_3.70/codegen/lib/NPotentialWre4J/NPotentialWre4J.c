 ...
 * Return Type  : void
 */
void NPotentialWre4J(const double TwMag[28], const double LiMas[28], const
                     double PoAcc[3], double Nt[4])
{
  double c_PoE[36];
  double JstS[24];
  double Wrench[24];
  ...
  for (i = 0; i < 4; i++) {
    ...
    for (c_i = 0; c_i < i; c_i++) {
      ...
      memcpy(&PoE[0], &b_PoE[0], 16U * sizeof(double));
    }
  ...
  for (i = 0; i < 4; i++) {
    memset(&JstS[0], 0, 24U * sizeof(double));
...
/*
 * File trailer for NPotentialWre4J.c
 *
 * [EOF]
 */
