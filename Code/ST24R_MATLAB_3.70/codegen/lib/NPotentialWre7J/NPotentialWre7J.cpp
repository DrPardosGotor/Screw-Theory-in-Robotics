...
// Return Type  : void
//
void NPotentialWre7J(const double TwMag[49], const double LiMas[49], const
                     double PoAcc[3], double Nt[7])
{
  double JstS[42];
  double Wrench[42];
  double tmp_data[42];
  ...
  for (i = 0; i < 7; i++) {
    int tmp_size_idx_1;
    std::memset(&JstS[0], 0, 42U * sizeof(double));
...
//
// File trailer for NPotentialWre7J.cpp
//
// [EOF]
//
